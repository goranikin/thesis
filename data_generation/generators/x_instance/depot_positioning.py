...
from data_generation.generators.x_instance.utils import rowcol_to_idx
...
def get_depot_idx(mode, grid):
    if mode == 'center':
        ...
        row = int(np.random.randint(grid[0], grid[1]))
        col = int(np.random.randint(grid[0], grid[1]))
        idx = rowcol_to_idx(row, col, grid)
    elif mode == 'quadrant':
        center_pos = int(0.5 * (grid[1] - grid[0]))
        row = int(np.random.randint(grid[0], center_pos))
        col = int(np.random.randint(grid[0], center_pos))
        idx = rowcol_to_idx(row, col, grid)
    else:
